...
app.post("/register", (req, res) => {
  ...
  console.log(
    "The email that has been given for registration: ",
    req.body.email
  );
  console.log(
    "The password that has been given for registration: ",
    req.body.password
  );
  db.transaction((trx) => {
    trx
    ...
    .then((data) => {
      ...
      db("products")
        ...
        .then((products) => {
          ...
          db.schema
            .createTable(`_${products[products.length - 1].id}`, (table) => {
              table.string("date");
...
app.post("/updateRecord/:id", (req, res) => {
  ...
  db.select("*")
    ...
    .then((data) => {
      ...
      if (data.length !== 0) {
        stock =
          Number(data[data.length - 1].stock) + Number(produced) - Number(sold);
      } else {
        stock = produced - sold;
      }
